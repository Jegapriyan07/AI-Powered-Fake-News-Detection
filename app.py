...
# --- App Setup ---
app = Flask(__name__)
# Allow requests from your frontend (We rely on Vercel's domain for deployed version)
CORS(app, resources={r"/*": {"origins": ["http://127.0.0.1:5500", "http://localhost:5500", "https://*.vercel.app"]}})

# --- AI Configuration ---
try:
    # 1. SECURITY FIX: Get API key from environment variable (Securely)
    ...
    # 2. STABILITY FIX: Use the specific stable version to prevent 404 Model Not Found errors
...
def call_gemini(model, system_prompt, user_query):
    # Check if model failed to configure (due to missing API key or bad model name)
    if not model:
        return "Error: AI model not configured correctly (API key missing in Vercel settings or incorrect model name).", None
